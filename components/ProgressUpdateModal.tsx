                ...
                return (
                  ...
                  >
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 space-y-3 shadow-sm">
                      {item.summary && <div className="text-[13px] font-medium text-slate-700 dark:text-slate-300 leading-relaxed"><FileText size={13} className="inline mr-1 opacity-50"/> {item.summary}</div>}
                      {(item.issues || item.nextSteps) && <div className="grid grid-cols-1 gap-2 mt-3 pt-3 border-t border-slate-200 dark:border-slate-800/50">
                        {item.issues && <div className="text-[12px] text-rose-700 dark:text-rose-400 font-medium"><AlertCircle size={12} className="inline mr-1 mb-0.5 text-rose-500"/> {item.issues}</div>}
                        {item.nextSteps && <div className="text-[12px] text-emerald-700 dark:text-emerald-400 font-medium"><CheckCircle2 size={12} className="inline mr-1 mb-0.5 text-emerald-500"/> {item.nextSteps}</div>}
                      </div>}
                    </div>
                  </div>
                );
