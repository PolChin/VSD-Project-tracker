...
const QuickWinsBoard: React.FC<QuickWinsBoardProps> = ({ masterData }) => {
  ...
  const [category, setCategory] = useState<'On-site Issue Solving' | 'Process Improvement'>('On-site Issue Solving');
  ...
  const filteredQuickWins = useMemo(() => {
    return quickWins.filter(win => {
      ...
      const matchDateStart = !dueDateStart || win.dueDate >= dueDateStart;
      const matchDateEnd = !dueDateEnd || win.dueDate <= dueDateEnd;
  ...
  const handleOpenCreate = () => {
    ...
    setCustomDeptText('');
    setStatus('Backlog');
    setCategory('On-site Issue Solving');
    setErrors({});
  ...
  const isOverdue = (dateStr: string, currentStatus: string) => {
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-850 dark:text-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                    >
                ...
                <div>
                  ...
                  {isCustomAssignee ? (
                    <input
                      ...
                      onChange={(e) => setCustomAssigneeText(e.target.value)}
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
                    />
                  ) : (
                    <select
